...
if (isset($_POST['addpet'])) {
  ...
  if (isset($_POST['edit_pet'])) {
    ...
    $update_pet = "UPDATE products SET plat_nomor = '$plat_nomor', type = 1, product_name = '$pet_name', product_details = '$pet_desc', product_price = '$pet_price' WHERE plat_nomor = '$edit_plat'";
    $result = $conn->query($update_pet);
    if ($result === TRUE) {
      ?>
      <script>
        window.location = "table_mpv.php";
      </script>
      <?php
    }
    exit;
  ...
  if ($plat_nomor != NULL && $pet_name != NULL && $pet_desc != NULL && $pet_price != NULL) {
    $target_dir = "admin/uploads/mobil/MPV/";
    $additional_target_dir = "../../Public/Index_Online/admin/uploads/mobil/MPV/";
    $date = date_create();
    ...
    if ($uploadOk == 0) {
      ...
      if (move_uploaded_file($_FILES["pet_image"]["tmp_name"], $target_file)) {
        ...
        $insert_pet = "INSERT INTO products (plat_nomor, type, product_name, product_details, product_price, image) 
                       VALUES ('$plat_nomor', 1, '$pet_name', '$pet_desc', '$pet_price', '$target_file')";
        if ($conn->query($insert_pet) === TRUE) {
          ?>
          <script>
            window.location = 'add_mpv.php'; 
          </script>
          <?php
        } else {
